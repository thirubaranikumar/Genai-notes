# -*- coding: utf-8 -*-
"""User intent - OpenAIEmbedding.ipynb
...
# Function to get OpenAI embeddings using the httpx library and the token
def get_openai_embeddings_with_token(texts, model="text-embedding-ada-002"):
    ...
    with httpx.Client() as client:
        ...
        if access_token:
            ...
            # Replace with your actual OpenAI embeddings endpoint URL
            ...
            try:
                ...
                embeddings = [item['embedding'] for item in embeddings_data.get('data', [])]
                return np.array(embeddings)
            except httpx.RequestError as e:
                print(f"Error generating embeddings: {e}")
                return None
            except Exception as e:
                print(f"Error processing embeddings response: {e}")
                return None
        else:
            print("\nCould not proceed with generating embeddings as access token was not obtained.")
            return None

# Example usage (can be removed or commented out if only the function is needed)
# if __name__ == "__main__":
#     text_to_embed = ["Your text string to embed", "Another text string"]
#     embeddings = get_openai_embeddings_with_token(text_to_embed)
...
# -----------------------------
# Step 2: Load embedding model and create embeddings for all examples
# -----------------------------
# Replace with your OpenAI API key or configure it securely
...
if embeddings is None:
    ...
    exit() # Exit if embeddings could not be generated
...
def predict_intent(query, threshold=0.6):
    ...
    if query_emb is None:
        return {"intent": "error", "confidence": 0.0, "match_examples": []}
